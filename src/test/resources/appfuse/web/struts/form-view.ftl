...
<div class="col-sm-12">
    <s:form id="${pojoNameLower}Form" action="save${pojo.shortName}" method="post" validate="false" cssClass="well" onsubmit="return ajaxSubmitFormUpdateAreas(this,undefined,afterSave${pojo.shortName});">
<#rt/>
...
<#elseif !c2h.isCollection(field) && !c2h.isManyToOne(field) && !c2j.isComponent(field)>
    <#foreach column in field.getColumnIterator()>
        <#if field.value.typeName == "java.util.Date" || field.value.typeName == "date">
            ...
            <#lt/>        <s:textfield cssClass="form-control date" key="${pojoNameLower}.${field.name}" size="11" title="date" datepicker="true"/>
        ...
        <#else>
            <#lt/>        <div class="form-group col-sm-12">
				        	  <label><fmt:message key="${pojoNameLower}.${field.name}"/></label>
				        	  <input class="form-control" type="text" name="${pojoNameLower}.${field.name}" value="${"$"}{${pojoNameLower}.${field.name}}"/>
				          </div>
            			<!--  <s:textfield cssClass="form-control" key="${pojoNameLower}.${field.name}"/> -->
        </#if>
    </#foreach>
        ...
        <div class="form-group">
            ...
            <c:if test="${'$'}{not empty ${pojoNameLower}.${idFieldName}}">
                <s:submit type="button" id="delete" cssClass="btn btn-danger" method="delete" key="button.delete"
                    onclick="${'$'}('#${pojoNameLower}Form').attr('action','/save${pojo.shortName}?delete=1'); return confirmMessage('确认删除？')" theme="simple">
