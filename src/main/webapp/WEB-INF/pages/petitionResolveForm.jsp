...
<%@ page import="com.joker.dq.model.PetitionFlow,java.util.List" %>
...
<div class="col-sm-12">
    <s:form id="petitionForm" action="saveResolvePetition" method="post" validate="false" cssClass="well" onsubmit="return ajaxSubmitFormUpdateAreas(this,undefined,afterSavePetition);">
        ...
        <table class="table table-condensed table-striped table-hover table-bordered">
        	...
        	<tr>
        		...
        		<td colspan="7">${petition.leaderContent}</td>
        	</tr>
        	        	<tr>
        		<th>处理结果</th>
        		<td colspan="7">
<%
List petitionFlowList = (List)request.getAttribute("petitionFlowList"); 
if(petitionFlowList!=null && petitionFlowList.size()>0){
	for(int i=0;i<petitionFlowList.size();i++){
		PetitionFlow petitionFlow = (PetitionFlow)petitionFlowList.get(i);
		if(petitionFlow.getSvcId()==5l){
%>
				<%=petitionFlow.getCreatedTime() %>:<%=petitionFlow.getMemo() %><br>
<%
		}
	}
}
%>
		...
		<div class="form-group col-sm-12">
       	  <label>处理结果</label>
       	  <textarea class="form-control" name="resolveresult" rows="5" cols="">${resolveresult}</textarea>
        </div>
