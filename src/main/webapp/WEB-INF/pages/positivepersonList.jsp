		...
		<tbody>
			<c:forEach items="${persons}" var="person">
				<tr onclick="commonTableListSelect(this,afterSelectPerson)">
					<td><a class="btn btn-info btn-sm" href="#" onclick="ajaxLoadDaialog({url:'<c:url value='/dqd/editPositivePerson'/>',title:'<fmt:message key="personDetail.heading"/>',width:1100,height:500,beforeDialogOpen:undefined,afterDialogOpen:afterPersonFormOpen,data:{personId:'${person.personId}',from:'list'}})">${person.personId}</a></td>
					<td><a class="btn btn-info btn-sm" href="#" onclick="ajaxLoadDaialog({url:'<c:url value='/dqd/approvalDangPerson'/>',title:'发展党员信息',width:500,height:350,beforeDialogOpen:undefined,afterDialogOpen:undefined,data:{partyId:'${person.partyId}',personId:'${person.personId}',from:'list'}})">${person.name}</a></td>
					<td>${person.cardId}</td>
					<td>${person.gender}</td>
					<td>${person.phone}</td>
					<td>
						<c:if test="${cpDtlMap[person.partyId].statusId eq '4'}">积极分子</c:if>
						<c:if test="${cpDtlMap[person.partyId].statusId eq '3'}">发展对象</c:if>
						<c:if test="${cpDtlMap[person.partyId].statusId eq '2'}">预备党员</c:if>
					</td>
				</tr>
			</c:forEach>
		</tbody>
