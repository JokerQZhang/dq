...
<#else>
    ...
    public String list() {
        try {
            ...
            ${util.getPluralForWord(pojoNameLower)} = ${pojoNameLower}Manager.search(condition, ${pojo.shortName}.class, getPage());
        } catch (SearchException se) {
            addActionError(se.getMessage());
            ${util.getPluralForWord(pojoNameLower)} = ${pojoNameLower}Manager.getAll(getPage());
        }
        if(query != null){
    ...
    public String save() throws Exception {
        ...
        ${pojoNameLower} = ${pojoNameLower}Manager.save(${pojoNameLower});
